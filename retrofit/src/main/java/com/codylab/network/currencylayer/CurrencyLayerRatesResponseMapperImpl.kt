...
internal class CurrencyLayerRatesResponseMapperImpl(
    ...
    override fun mapToRates(response: LiveRatesResponse): List<Rate> {
        return response.quotes?.entries?.map {
            ...
            val toCurrency = currencyRepository.lookupCurrency(toCurrencyCode) ?: return@map null
            val fromCurrency = currencyRepository.lookupCurrency(fromCurrencyCode) ?: return@map null
            val rate = it.value
